...
if (!empty($_POST["save"])) {
	...
	if (!empty($_POST["v_options"])) {
		...
		fclose($fp);
		exec(
			LINKPANEL_CMD . "v-change-sys-service-config " . $new_conf . " bind9-opt " . $v_restart,
			$output,
			$return_var,
		);
	...
	if (empty($_SESSION["error_msg"]) && !empty($_POST["v_config"])) {
		...
		fclose($fp);
		exec(
			LINKPANEL_CMD . "v-change-sys-service-config " . $new_conf . " bind9 " . $v_restart,
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
...
// Read config
$v_options = shell_exec(LINKPANEL_CMD . "v-open-fs-config " . $v_options_path);
