...
if (!empty($_GET["domain"]) && empty($_GET["account"])) {
	...
	exec(LINKPANEL_CMD . "v-list-sys-webmail json", $output, $return_var);
...
if (!empty($_POST["save"]) && !empty($_GET["domain"]) && empty($_GET["account"])) {
	...
	if ($v_dkim == "no" && !empty($_POST["v_dkim"]) && empty($_SESSION["error_msg"])) {
		exec(
			LINKPANEL_CMD . "v-add-mail-domain-dkim " . $v_username . " " . quoteshellarg($v_domain),
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if (!empty($_POST["v_reject"]) && $v_antispam == "yes" && $v_reject != "yes") {
		exec(
			LINKPANEL_CMD . "v-add-mail-domain-reject " . $user . " " . $v_domain . " yes",
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if (empty($_POST["v_reject"]) && $v_reject == "yes") {
		exec(
			LINKPANEL_CMD . "v-delete-mail-domain-reject " . $user . " " . $v_domain . " yes",
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if (!empty($_SESSION["IMAP_SYSTEM"]) && !empty($_SESSION["WEBMAIL_SYSTEM"])) {
		if (empty($_POST["v_webmail"])) {
			if (empty($_SESSION["error_msg"])) {
				exec(
					LINKPANEL_CMD . "v-delete-mail-domain-webmail " . $user . " " . $v_domain . " yes",
					$output,
					$return_var,
				);
				check_return_code($return_var, $output);
	...
	if ($v_ssl == "yes" && empty($_POST["v_ssl"]) && empty($_SESSION["error_msg"])) {
		exec(
			LINKPANEL_CMD . "v-delete-mail-domain-ssl " . $v_username . " " . quoteshellarg($v_domain),
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
