...
if [[ $REPLY =~ ^[Yy]$ ]]; then
	...
	mkdir -p /root/linkpnl_backup_man/var_phmyadmin
	cp -r /var/lib/phpmyadmin/* /root/linkpnl_backup_man/var_phmyadmin
	...
	if [ "$WEB_SYSTEM" == "apache2" ]; then
		cp -f $LINKPANEL_INSTALL_DIR/pma/apache.conf /etc/phpmyadmin/
...
MYSQL_PMA1
	...
	mysql -uroot < $LINKPANEL_INSTALL_DIR/phpmyadmin/create_tables.sql
