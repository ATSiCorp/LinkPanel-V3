...
# LinkPanel Control Panel upgrade script for target version 1.5.0
...
if [ -n "$MAIL_SYSTEM" ]; then
	...
	for ip in $($BIN/v-list-sys-ips plain | cut -f1); do
		sed '/^HELO/d' $LINKPANEL/data/ips/$ip > /dev/null
	done
fi

if [ -L "/var/log/linkpanel" ]; then
	echo "[ ! ] Updating log file location: /usr/local/linkpanel/log/* to /var/log/linkpanel/..."
	rm /var/log/linkpanel
	mkdir -p /var/log/linkpanel
	cp /usr/local/linkpanel/log/* /var/log/linkpanel/
	rm -rf /usr/local/linkpanel/log
	ln -s /var/log/linkpanel /usr/local/linkpanel/log
	touch /var/log/linkpanel/auth.log /var/log/linkpanel/error.log /var/log/linkpanel/system.log /var/log/linkpanel/nginx-error.log /var/log/linkpanel/nginx-access.log
fi
