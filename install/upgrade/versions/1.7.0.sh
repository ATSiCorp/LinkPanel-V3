...
# LinkPanel Control Panel upgrade script for target version 1.7.0
...
if [ "$FILE_MANAGER" = "true" ]; then
	...
	$LINKPANEL/bin/v-delete-sys-filemanager quiet
...
for package in $packages; do
	if [ -z "$(grep -e 'RATE_LIMIT' $LINKPANEL/data/packages/$package)" ]; then
		echo "RATE_LIMIT='200'" >> $LINKPANEL/data/packages/$package
	fi
done
...
for file in $LINKPANEL/data/templates/web/php-fpm/*; do
	echo "[ * ] Update $file legacy /var/run/ to /run/..."
	sed -i 's|/var/run/|/run/|g' $file
done
...
if [ "$PHPMYADMIN_KEY" != "" ]; then
	echo "[ * ] Refresh linkpanel-sso for PMA..."
