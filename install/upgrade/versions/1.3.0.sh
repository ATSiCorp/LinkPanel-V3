...
# LinkPanel Control Panel upgrade script for target version 1.3.0
...
if [ ! -e "/etc/cron.d/linkpanel-proc" ]; then
...
# Set LOGIN_STYLE variable in linkpanel.conf
echo "[ * ] Updating configuration file: linkpanel.conf..."
$BIN/v-change-sys-config-value "LOGIN_STYLE" "default"

# Remove old lanugage files.
if [ -e $LINKPANEL/web/inc/i18n/en.php ]; then
	echo "[ * ] Performing language library clean-up..."
	rm -fr $LINKPANEL/web/inc/i18n
fi
