...
# LinkPanel Control Panel upgrade script for target version 1.4.13
...
# Remove old files/folders from previous versions of LinkPanel Control Panel
if [ -d "$LINKPANEL/web/edit/file/" ]; then
	rm -fr $LINKPANEL/web/edit/file/
fi
if [ -d "$LINKPANEL/web/edit/server/theme/" ]; then
	rm -fr $LINKPANEL/web/edit/server/theme/
fi
