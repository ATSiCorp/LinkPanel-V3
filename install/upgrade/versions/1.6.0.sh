...
if [ "$MAIL_SYSTEM" = "exim4" ]; then
	...
	cp $LINKPANEL_INSTALL_DIR/exim/limit.conf /etc/exim4/limit.conf
	cp $LINKPANEL_INSTALL_DIR/exim/system.filter /etc/exim4/system.filter
...
cp -rf $LINKPANEL_INSTALL_DIR/api $LINKPANEL/data/
...
if [ $release = 'Ubuntu' ]; then
	if [ $ips -gt 1 ]; then
		add_upgrade_message "Warning: Please check your network configuration!\n\n A bug has been discovered that might affect your setup and can lead to issues after a system reboot. Please review your network configuration. See https://github.com/ATSiCorp/LinkPanel-V3/pull/2612#issuecomment-1135571835 for more info regarding this issue!"
		$LINKPANEL/bin/v-add-user-notification admin "Warning: Please check your network configuration!\n\n A bug has been discovered that might affect your setup and can lead to issues after a system reboot. Please review your network configuration. <a href='https://github.com/ATSiCorp/LinkPanel-V3/pull/2612#issuecomment-1135571835'>More info</a>"
	fi
fi
