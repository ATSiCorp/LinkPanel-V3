...
# LinkPanel Control Panel upgrade script for target version 1.4.0
...
if [ "$API" = "no" ]; then
	if [ -f "$LINKPANEL/web/api/index.php" ]; then
		...
		$LINKPANEL/bin/v-change-sys-api remove
	fi
fi

# Back up users existing configuration data to $LINKPANEL/conf/defaults/linkpanel.conf
if [ ! -f "$LINKPANEL/conf/defaults/linkpanel.conf" ]; then
	...
	cp -f $LINKPANEL/conf/linkpanel.conf $LINKPANEL/conf/defaults/linkpanel.conf
fi
...
if [ "$WEB_SYSTEM" = "nginx" ]; then
	...
	sed -i "s|TPL='drupal6'|TPL='drupal'|g" $LINKPANEL/data/users/*/web.conf
	sed -i "s|TPL='drupal7'|TPL='drupal'|g" $LINKPANEL/data/users/*/web.conf
	sed -i "s|TPL='drupal8'|TPL='drupal'|g" $LINKPANEL/data/users/*/web.conf
	sed -i "s|TPL='codeigniter2'|TPL='codeigniter'|g" $LINKPANEL/data/users/*/web.conf
	sed -i "s|TPL='codeigniter3'|TPL='codeigniter'|g" $LINKPANEL/data/users/*/web.conf
fi

# Remove outdated nginx templates
echo "[ * ] Removing outdated nginx templates..."
rm -rf $LINKPANEL/data/templates/web/nginx/php-fpm/drupal6.*tpl
rm -rf $LINKPANEL/data/templates/web/nginx/php-fpm/drupal7.*tpl
rm -rf $LINKPANEL/data/templates/web/nginx/php-fpm/drupal8.*tpl
rm -rf $LINKPANEL/data/templates/web/nginx/php-fpm/codeigniter2.*tpl
rm -rf $LINKPANEL/data/templates/web/nginx/php-fpm/codeigniter3.*tpl

# Clean up old LinkPanel controlled webapps
if [ -d "$LINKPANEL/web/images/webapps/" ]; then
	echo "[ * ] Clean up old web apps code..."
	rm -rf $LINKPANEL/web/images/webapps/
	rm -rf $LINKPANEL/web/src/app/WebApp/Installers/LaravelSetup.php
	rm -rf $LINKPANEL/web/src/app/WebApp/Installers/OpencartSetup.php
	rm -rf $LINKPANEL/web/src/app/WebApp/Installers/PrestashopSetup.php
	rm -rf $LINKPANEL/web/src/app/WebApp/Installers/SymfonySetup.php
	rm -rf $LINKPANEL/web/src/app/WebApp/Installers/WordpressSetup.php
	rm -rf $LINKPANEL/web/src/app/WebApp/Installers/Joomla
fi
