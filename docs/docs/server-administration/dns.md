...
## Host DNS for your domain on LinkPanel

Pre-requisites

These steps require that you configure the DNS servers of your domain to use your LinkPanel servers.
...
1. On your LinkPanel master, [create a DNS Zone](../user-guide/dns#adding-a-dns-zone) with the **child-ns** template
2. On your domain registrar panel, set the name servers of the domain to the LinkPanel servers
...
A Master server is where DNS zones are created, and a Slave server recieves the zone via the API. LinkPanel can be configured as Master <-> Master or Master -> Slave. With a Master <-> Master configuration, each Master is also a Slave, so it could be considered as Master/Slave <-> Master/Slave.
...
### Master <-> Master DNS cluster (Default setup) with the LinkPanel API

::: warning
This method does not support DNSSEC!
:::

1. Create a new user on each LinkPanel server that will act as a “Slave”. Make sure it uses the username of "dns-cluster" or has the role `dns-cluster`
...
### Master -> Slave DNS cluster with the LinkPanel API

::: info
It doesn't work if you try to sync via local network! See [Issue](https://github.com/ATSiCorp/LinkPanel-V3/issues/4295) Make sure to use the public ip addresses
:::
...
5. Edit `/usr/local/linkpanel/conf/linkpanel.conf`, change `DNS_CLUSTER_SYSTEM='linkpanel'` to `DNS_CLUSTER_SYSTEM='linkpanel-zone'`.
...
1. In `/usr/local/linkpanel/conf/linkpanel.conf`, change `DNS_CLUSTER_SYSTEM='linkpanel'` to `DNS_CLUSTER_SYSTEM='linkpanel-zone'`.
...
::: warning
DNSSEC can’t be used when LinkPanel Cluster is active as Master <-> Master
:::
...
::: danger
Removing or disabling the private key in LinkPanel will make the domain inaccessble.
:::
...
```bash
/usr/local/linkpanel/func/remote.sh: line 43: return: Error:: numeric argument required
