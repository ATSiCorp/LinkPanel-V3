...
#
# Script for preparing lxd environment and building LinkPanel packages for all supported distros
...
function setup_container() {
	...
	lxc config device add ${containername} debdir disk path=/opt/linkpanelcp source=${__DIR__}/build/${containername}
}
...
for osdef in "${oslist[@]}"; do
	...
	for osver in $osversions; do
		...
		containername="linkpnl-${osname}-${osver/\./}"
		container_ip=""
		...
		if [ "$cmd" = '--background' ]; then
			...
			lxc exec $containername -- /opt/linkpanelcp/lxd_compile.sh $branch > /dev/null 2>&1 &
		else
			lxc exec $containername -- /opt/linkpanelcp/lxd_compile.sh $branch
		fi
