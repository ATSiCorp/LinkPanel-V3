...
const { config } = JSON.parse(
	execSync(`${process.env.LINKPANEL}/bin/v-list-sys-config json`, { silent: true }).toString(),
);
...
wss.on('connection', (ws, req) => {
	...
	const file = readFileSync(`${process.env.LINKPANEL}/data/sessions/sess_${sessionID}`);
	if (!file) {
	...
	const pty = spawn(shell, [], {
		...
		env: {
			...
			HOME: homedir,
			PWD: homedir,
			LINKPANEL: process.env.LINKPANEL,
		},
	});
