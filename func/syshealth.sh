...
#===========================================================================#
#                                                                           #
# LinkPanel Control Panel - System Health Check and Repair Function Library    #
#                                                                           #
#===========================================================================#

# Read known configuration keys from $LINKPANEL/conf/defaults/$system.conf
function read_kv_config_file() {
	local system=$1

	if [ ! -f "$LINKPANEL/conf/defaults/$system.conf" ]; then
		write_kv_config_file $system
	fi
	while read -r str; do
		echo "$str"
	done < <(cat $LINKPANEL/conf/defaults/$system.conf)
	unset system
}

# Write known configuration keys to $LINKPANEL/conf/defaults/
function write_kv_config_file() {
	# Ensure configuration directory exists
	if [ ! -d "$LINKPANEL/conf/defaults/" ]; then
		mkdir "$LINKPANEL/conf/defaults/"
	fi

	# Remove previous known good configuration
	if [ -f "$LINKPANEL/conf/defaults/$system.conf" ]; then
		rm -f $LINKPANEL/conf/defaults/$system.conf
	fi

	touch $LINKPANEL/conf/defaults/$system.conf
	for key in $known_keys; do
		echo $key >> $LINKPANEL/conf/defaults/$system.conf
	done
}
...
function syshealth_update_web_config_format() {
	...
	system="web"
	known_keys="DOMAIN IP IP6 CUSTOM_DOCROOT CUSTOM_PHPROOT FASTCGI_CACHE FASTCGI_DURATION ALIAS TPL SSL SSL_FORCE SSL_LINKPNLS SSL_HOME LETSENCRYPT FTP_USER FTP_MD5 FTP_PATH BACKEND PROXY PROXY_EXT STATS STATS_USER STATS_CRYPT REDIRECT REDIRECT_CODE AUTH_USER AUTH_HASH SUSPENDED TIME DATE"
...
# Replaces $LINKPANEL/conf/linkpanel.conf with "known good defaults" file ($LINKPANEL/conf/defaults/linkpanel.conf)
function syshealth_restore_system_config() {
	if [ -f "$LINKPANEL/conf/defaults/linkpanel.conf" ]; then
		mv $LINKPANEL/conf/linkpanel.conf $LINKPANEL/conf/linkpanel.conf.old
		cp $LINKPANEL/conf/defaults/linkpanel.conf $LINKPANEL/conf/linkpanel.conf
		rm -f $LINKPANEL/conf/linkpanel.conf.old
	else
...
function check_key_exists() {
	grep -e "^$1=" $LINKPANEL/conf/linkpanel.conf
}

# Repair System Configuration
# Adds missing variables to $LINKPANEL/conf/linkpanel.conf with safe default values
function syshealth_repair_system_config() {
	# Release branch
	if [[ -z $(check_key_exists 'RELEASE_BRANCH') ]]; then
		echo "[ ! ] Adding missing variable to linkpanel.conf: RELEASE_BRANCH ('release')"
		$BIN/v-change-sys-config-value 'RELEASE_BRANCH' 'release'
	fi
	# Webmail alias
	if [ -n "$IMAP_SYSTEM" ]; then
		if [[ -z $(check_key_exists 'WEBMAIL_ALIAS') ]]; then
			echo "[ ! ] Adding missing variable to linkpanel.conf: WEBMAIL_ALIAS ('webmail')"
	...
	if [ -n "$DB_SYSTEM" ]; then
		if [ "$DB_SYSTEM" = "mysql" ]; then
			if [[ -z $(check_key_exists 'DB_PMA_ALIAS') ]]; then
				echo "[ ! ] Adding missing variable to linkpanel.conf: DB_PMA_ALIAS ('phpmyadmin)"
				$BIN/v-change-sys-config-value 'DB_PMA_ALIAS' 'phpmyadmin'
			fi
		fi
		if [ "$DB_SYSTEM" = "pgsql" ]; then
			if [[ -z $(check_key_exists 'DB_PGA_ALIAS') ]]; then
				echo "[ ! ] Adding missing variable to linkpanel.conf: DB_PGA_ALIAS ('phppgadmin')"
	...
	if [[ -z $(check_key_exists 'BACKUP_GZIP') ]]; then
		echo "[ ! ] Adding missing variable to linkpanel.conf: BACKUP_GZIP ('4')"
		$BIN/v-change-sys-config-value 'BACKUP_GZIP' '4'
	fi

	# Theme
	if [[ -z $(check_key_exists 'THEME') ]]; then
		echo "[ ! ] Adding missing variable to linkpanel.conf: THEME ('dark')"
		$BIN/v-change-sys-config-value 'THEME' 'dark'
	fi

	# Default language
	if [[ -z $(check_key_exists 'LANGUAGE') ]]; then
		echo "[ ! ] Adding missing variable to linkpanel.conf: LANGUAGE ('en')"
		$BIN/v-change-sys-language 'LANGUAGE' 'en'
	fi

	# Disk Quota
	if [[ -z $(check_key_exists 'DISK_QUOTA') ]]; then
		echo "[ ! ] Adding missing variable to linkpanel.conf: DISK_QUOTA ('no')"
		$BIN/v-change-sys-config-value 'DISK_QUOTA' 'no'
	fi

	# CRON daemon
	if [[ -z $(check_key_exists 'CRON_SYSTEM') ]]; then
		echo "[ ! ] Adding missing variable to linkpanel.conf: CRON_SYSTEM ('cron')"
		$BIN/v-change-sys-config-value 'CRON_SYSTEM' 'cron'
	fi

	# Backend port
	if [[ -z $(check_key_exists 'BACKEND_PORT') ]]; then
		ORIGINAL_PORT=$(sed -ne "/listen/{s/.*listen[^0-9]*\([0-9][0-9]*\)[ \t]*ssl\;/\1/p;q}" "$LINKPANEL/nginx/conf/nginx.conf")
		echo "[ ! ] Adding missing variable to linkpanel.conf: BACKEND_PORT ('$ORIGINAL_PORT')"
	...
	if [[ -z $(check_key_exists 'UPGRADE_SEND_EMAIL') ]]; then
		echo "[ ! ] Adding missing variable to linkpanel.conf: UPGRADE_SEND_EMAIL ('true')"
		$BIN/v-change-sys-config-value 'UPGRADE_SEND_EMAIL' 'true'
	fi

	# Upgrade: Send email notification
	if [[ -z $(check_key_exists 'UPGRADE_SEND_EMAIL_LOG') ]]; then
		echo "[ ! ] Adding missing variable to linkpanel.conf: UPGRADE_SEND_EMAIL_LOG ('false')"
		$BIN/v-change-sys-config-value 'UPGRADE_SEND_EMAIL_LOG' 'false'
	fi

	# File Manager
	if [[ -z $(check_key_exists 'FILE_MANAGER') ]]; then
		echo "[ ! ] Adding missing variable to linkpanel.conf: FILE_MANAGER ('true')"
	...
	if [[ -z $(check_key_exists 'LOGIN_STYLE') ]]; then
		echo "[ ! ] Adding missing variable to linkpanel.conf: LOGIN_STYLE ('default')"
	...
	if [[ -z $(check_key_exists 'WEBMAIL_SYSTEM') ]]; then
		if [ -d "/var/lib/roundcube" ]; then
			echo "[ ! ] Adding missing variable to linkpanel.conf: WEBMAIL_SYSTEM ('roundcube')"
			$BIN/v-change-sys-config-value "WEBMAIL_SYSTEM" "roundcube"
		else
			echo "[ ! ] Adding missing variable to linkpanel.conf: WEBMAIL_SYSTEM ('')"
	...
	if [[ -z $(check_key_exists 'INACTIVE_SESSION_TIMEOUT') ]]; then
		echo "[ ! ] Adding missing variable to linkpanel.conf: INACTIVE_SESSION_TIMEOUT ('60')"
	...
	if [[ -z $(check_key_exists 'ENFORCE_SUBDOMAIN_OWNERSHIP') ]]; then
		echo "[ ! ] Adding missing variable to linkpanel.conf: ENFORCE_SUBDOMAIN_OWNERSHIP ('yes')"
		$BIN/v-change-sys-config-value "ENFORCE_SUBDOMAIN_OWNERSHIP" "yes"
	fi

	if [[ -z $(check_key_exists 'API') ]]; then
		echo "[ ! ] Adding missing variable to linkpanel.conf: API ('no')"
		$BIN/v-change-sys-config-value "API" "no"
	fi

	# Enable API V2
	if [[ -z $(check_key_exists 'API_SYSTEM') ]]; then
		echo "[ ! ] Adding missing variable to linkpanel.conf: API_SYSTEM ('0')"
		$BIN/v-change-sys-config-value "API_SYSTEM" "0"
	fi

	# API access allowed IP's
	if [ "$API" = "yes" ]; then
		check_api_key=$(grep "API_ALLOWED_IP" $LINKPANEL/conf/linkpanel.conf)
		if [ -z "$check_api_key" ]; then
			if [[ -z $(check_key_exists 'API_ALLOWED_IP') ]]; then
				echo "[ ! ] Adding missing variable to linkpanel.conf: API_ALLOWED_IP ('allow-all')"
	...
	if [[ -z $(check_key_exists 'POLICY_USER_DELETE_LOGS') ]]; then
		echo "[ ! ] Adding missing variable to linkpanel.conf: POLICY_USER_DELETE_LOGS ('yes')"
	...
	if [[ -z $(check_key_exists 'POLICY_USER_EDIT_DETAILS') ]]; then
		echo "[ ! ] Adding missing variable to linkpanel.conf: POLICY_USER_EDIT_DETAILS ('yes')"
	...
	if [[ -z $(check_key_exists 'POLICY_USER_EDIT_DNS_TEMPLATES') ]]; then
		echo "[ ! ] Adding missing variable to linkpanel.conf: POLICY_USER_EDIT_DNS_TEMPLATES ('yes')"
	...
	if [[ -z $(check_key_exists 'POLICY_USER_EDIT_WEB_TEMPLATES') ]]; then
		echo "[ ! ] Adding missing variable to linkpanel.conf: POLICY_USER_EDIT_WEB_TEMPLATES ('yes')"
		$BIN/v-change-sys-config-value "POLICY_USER_EDIT_WEB_TEMPLATES" "yes"
	fi
	# View user logs
	if [[ -z $(check_key_exists 'POLICY_USER_VIEW_LOGS') ]]; then
		echo "[ ! ] Adding missing variable to linkpanel.conf: POLICY_USER_VIEW_LOGS ('yes')"
	...
	while IFS='= ' read -r lhs rhs; do
		...
		check_ckey=$(grep "^$lhs='" "$LINKPANEL/conf/linkpanel.conf.new")
		if [ -z "$check_ckey" ]; then
			echo "$lhs='$rhs'" >> "$LINKPANEL/conf/linkpanel.conf.new"
		else
			sed -i "s|^$lhs=.*|$lhs='$rhs'|g" "$LINKPANEL/conf/linkpanel.conf.new"
		fi
	done < $LINKPANEL/conf/linkpanel.conf

	cmp --silent $LINKPANEL/conf/linkpanel.conf $LINKPANEL/conf/linkpanel.conf.new
	if [ $? -ne 0 ]; then
		echo "[ ! ] Duplicated keys found repair config"
		rm $LINKPANEL/conf/linkpanel.conf
		cp $LINKPANEL/conf/linkpanel.conf.new $LINKPANEL/conf/linkpanel.conf
		rm $LINKPANEL/conf/linkpanel.conf.new
	fi
...
function syshealth_adapt_linkpanel_nginx_listen_ports() {
	...
	NGINX_BCONF_CHANGED=""
	NGINX_BCONF="/usr/local/linkpanel/nginx/conf/nginx.conf"
...
syshealth_adapt_nginx_resolver() {
	NGINX_CONF="/usr/local/linkpanel/nginx/conf/nginx.conf"
