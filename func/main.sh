...
#===========================================================================#
#                                                                           #
# LinkPanel Control Panel - Core Function Library                              #
#                                                                           #
#===========================================================================#
...
log_event() {
	...
	if [ "$1" -eq 0 ]; then
		echo "$LOG_TIME $2" >> $LINKPANEL/log/system.log
	else
		echo "$LOG_TIME $2 [Error $1]" >> $LINKPANEL/log/error.log
	fi
}
...
log_history() {
	...
	if [ "$log_user" = "system" ]; then
		log=$LINKPANEL/log/activity.log
	else
		if ! $BIN/v-list-user "$log_user" > /dev/null; then
			return $E_NOTEXIST
		fi
		log=$LINKPANEL/data/users/$log_user/history.log
	fi
	touch $log
...
is_package_valid() {
	if [ -z $1 ]; then
		if [ ! -e "$LINKPANEL/data/packages/$package.pkg" ]; then
			check_result "$E_NOTEXIST" "package $package doesn't exist"
		fi
	else
		if [ ! -e "$LINKPANEL/data/packages/$1.pkg" ]; then
...
# Check user backup settings
is_backup_scheduled() {
	if [ -e "$LINKPANEL/data/queue/backup.pipe" ]; then
		check_q=$(grep " $user " $LINKPANEL/data/queue/backup.pipe | grep $1)
		if [ -n "$check_q" ]; then
...
is_object_valid() {
	if [ $2 = 'USER' ]; then
		tstpath="$(readlink -f "$LINKPANEL/data/users/$3")"
		if [ "$(dirname "$tstpath")" != "$(readlink -f "$LINKPANEL/data/users")" ] || [ ! -d "$LINKPANEL/data/users/$3" ]; then
			check_result "$E_NOTEXIST" "$1 $3 doesn't exist"
		fi
	elif [ $2 = 'KEY' ]; then
		local key="$(basename "$3")"

		if [[ -z "$key" || ${#key} -lt 16 ]] || [[ ! -f "$LINKPANEL/data/access-keys/${key}" && ! -f "$LINKPANEL/data/access-keys/$key" ]]; then
			check_result "$E_NOTEXIST" "$1 $3 doesn't exist"
		fi
	else
		object=$(grep "$2='$3'" $LINKPANEL/data/users/$user/$1.conf)
		if [ -z "$object" ]; then
			arg1=$(basename $1)
...
update_user_value() {
	key="${2//$/}"
	lnr=$(grep -n "^$key='" $LINKPANEL/data/users/$1/user.conf | cut -f 1 -d ':')
	if [ -n "$lnr" ]; then
		sed -i "$lnr d" $LINKPANEL/data/users/$1/user.conf
		sed -i "$lnr i\\$key='${3}'" $LINKPANEL/data/users/$1/user.conf
	fi
}
...
increase_user_value() {
	key="${2//$/}"
	factor="${3-1}"
	conf="$LINKPANEL/data/users/$1/user.conf"
...
check_access_key_secret() {
	...
	if [[ -z "$access_key_id" || ! -f "$LINKPANEL/data/access-keys/${access_key_id}" ]]; then
...
check_access_key_cmd() {
	...
	if [[ "$DEBUG_MODE" = "true" ]]; then
		new_timestamp
		echo "[$date:$time] $1 $2" >> /var/log/linkpanel/api.log
	fi
	if [[ -z "$access_key_id" || ! -f "$LINKPANEL/data/access-keys/${access_key_id}" ]]; then
		check_result "$E_FORBIDEN" "Access key $access_key_id doesn't exist"
	fi
...
change_sys_value() {
	check_ckey=$(grep "^$1='" "$LINKPANEL/conf/linkpanel.conf")
	if [ -z "$check_ckey" ]; then
		echo "$1='$2'" >> "$LINKPANEL/conf/linkpanel.conf"
	else
		sed -i "s|^$1=.*|$1='$2'|g" "$LINKPANEL/conf/linkpanel.conf"
	fi
}
...
is_key_permissions_format_valid() {
	...
	while IFS=',' read -ra permissions_arr; do
		for permission in "${permissions_arr[@]}"; do
			...
			if [[ ! -e "$LINKPANEL/data/api/$permission" ]]; then
				check_result "$E_NOTEXIST" "API $permission doesn't exist"
			fi

			source_conf "$LINKPANEL/data/api/$permission"
...
# Get the position of an argument by name in a linkpanel command using the command's documentation comment.
#
# Return:
